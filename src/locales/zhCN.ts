...
export default {
	...
	general: {
		...
		confirmOverwriteFile:
			'此操作会覆盖同名文件。你要继续吗？',
		fileSystemPolyfill:
			'由于你的浏览器原因，你需要将你的项目下载到本地来实际保存你的进度。如果你使用的是Chrome或Edge，就无需这么做了！',
		successfulExport: {
			title: '导出成功',
	...
	windows: {
		...
		packExplorer: {
			...
			restartDevServer: {
				name: '重启开发服务器',
				description:
					"您确定要重新启动编译器的开发服务器吗？这可能需要一些时间，具体取决于项目的大小。",
			},
			createPreset: '新建文件',
			projectConfig: {
				name: '打开项目配置',
		...
		browserUnsupported: {
			title: '不支持的浏览器',
			description:
				'请使用Chrome（桌面版）或Edge（Chromium版）以获得使用bridge. v2的最佳体验！你的浏览器不支持直接保存文件以及将项目同步到你的com.mojang文件夹的功能。',
			continue: '依旧继续',
		},
		invalidJson: {
			title: '无效的JSON',
	...
	preview: {
		...
		chooseGeometry: '选择几何',
		noGeometry:
			'在此文件中找不到有效的几何。确保您的JSON有效、文件结构正确且具有所提供ID的几何存在。',
	},
	initialSetup: {
