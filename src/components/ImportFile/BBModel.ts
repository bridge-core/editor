...
export class BBModelImporter extends FileImporter {
	...
	async exportImages(app: App, textures: any[]) {
		for (let texture of textures) {
			const filePath = join('RP', 'textures', 'entity', texture.folder, texture.name)
			...
			const destHandle = await app.project.fileSystem.getFileHandle(filePath, true)

			const buffer = Buffer.from(texture.source.split(';base64,').pop(), 'base64')
			await app.project.fileSystem.writeFile(filePath, buffer)
	...
	compileGroup(data: any, group: any) {
		...
		bone.pivot[0] *= -1
		if (group.rotation) {
			if (group.rotation[0] !== 0 || group.rotation[1] !== 0 || group.rotation[2] !== 0) {
		...
		for (let child of group.children) {
			if (!(child instanceof Object)) {
				let element = data.elements.find((element: { uuid: string }) => element.uuid === child)
				if (element.type !== 'locator') {
					...
					if (element.rotation[0] !== 0 || element.rotation[1] !== 0 || element.rotation[2] !== 0) {
						locators[key] = {
							offset,
							rotation: [
								-element.rotation[0],
	...
	compileCube(data: any, element: any, bone: any) {
		let cube = {
			...
			size: [element.to[0] - element.from[0], element.to[1] - element.from[1], element.to[2] - element.from[2]],
	...
	calculateVisibleBox(data: any) {
		...
		elements.forEach(element => {
			...
			visible_box.max.x = Math.max(visible_box.max.x, element.from[0], element.to[0])
			visible_box.min.x = Math.min(visible_box.min.x, element.from[0], element.to[0])

			visible_box.max.y = Math.max(visible_box.max.y, element.from[1], element.to[1])
			visible_box.min.y = Math.min(visible_box.min.y, element.from[1], element.to[1])

			visible_box.max.z = Math.max(visible_box.max.z, element.from[2], element.to[2])
			visible_box.min.z = Math.min(visible_box.min.z, element.from[2], element.to[2])
		})
		...
		//Width
		let radius = Math.max(visible_box.max.x, visible_box.max.z, -visible_box.min.x, -visible_box.min.z)
